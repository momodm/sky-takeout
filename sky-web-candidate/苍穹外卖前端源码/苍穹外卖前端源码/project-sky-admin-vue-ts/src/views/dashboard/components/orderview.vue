<template>
  <div class="container">
    <h2 class="homeTitle">
      订单履约概览<i>{{ days[1] }}</i>
      <span><router-link to="/order">查看订单中心</router-link></span>
    </h2>
    <div class="orderviewBox">
      <ul>
        <li>
          <span class="status"><i class="iconfont icon-waiting"></i>待接单</span>
          <span class="num tip">
            <router-link to="/order?status=2">{{ orderviewData.waitingOrders }}</router-link>
          </span>
        </li>
        <li>
          <span class="status"><i class="iconfont icon-staySway"></i>待派送</span>
          <span class="num tip">
            <router-link to="/order?status=3">{{ orderviewData.deliveredOrders }}</router-link>
          </span>
        </li>
        <li>
          <span class="status"><i class="iconfont icon-complete"></i>已完成</span>
          <span class="num">
